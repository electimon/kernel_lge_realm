...
#define QFPROM_CTRL_BASE                0xFC4B8000
    ...
    #define QFPROM_SECONDARY_HW_KEY     0xFC4B8398
    ...
    #elif defined(CONFIG_MACH_MSM8X10_W5C_TRF_US)
        #define FUSING_COMPLETED_STATE 0x3F
        static struct qfprom_blow_data blow_data[] = {
                /* Don't change array order !!!!!!!!!!!!!!*/
                /* addr              LSB        MSB*/
            { QFPROM_OEM_CONFIG,        0x00310225,     0x0000000F},        /* OEM ID + ProductID + Anti-Rollback */
            { QFPROM_SECURE_BOOT_ENABLE,    0x00202020,     0x00000000},        /* SECURE ENABLE */
            { QFPROM_DEBUG_DISABLE,     0x3FC000C0,     0x002401FE},        /* JTAG DISABLE + WDOG_EN DISABLE  */
            { QFPROM_CHECK_HW_KEY,      0x0,            0x0   },
            { QFPROM_RD_PERMISSION,     0x00000000,     0x0000FC00},            /* READ WRITE PERMISSION */
            { QFPROM_WR_PERMISSION,     0x00300000,     0x00CFC030},            /* READ WRITE PERMISSION */
        };
        ...
        static struct qfprom_blow_data blow_data[] = {
                /* Don't change array order !!!!!!!!!!!!!!*/
                /* addr              LSB        MSB*/
            { QFPROM_OEM_CONFIG,        0x00310226,     0x0000000F},        /* OEM ID + ProductID + Anti-Rollback */
            { QFPROM_SECURE_BOOT_ENABLE,    0x00202020,     0x00000000},        /* SECURE ENABLE + WDOG_EN DISABLE */
            { QFPROM_DEBUG_DISABLE,     0x3FC000C0,     0x002401FE},        /* JTAG DISABLE   */
            { QFPROM_CHECK_HW_KEY,      0x0,            0x0   },
            { QFPROM_RD_PERMISSION,     0x00000000,     0x0000FC00},            /* READ WRITE PERMISSION */
            { QFPROM_WR_PERMISSION,     0x00300000,     0x00CFC030},            /* READ WRITE PERMISSION */
...
static ssize_t qfprom_rbvalue_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    u32 *p_buf = NULL;
    int ret = 0, i;
	u32 qfprom_address[2];

#if defined (CONFIG_ARCH_MSM8610)
	qfprom_address[0] = 0xFC4B80C0;	qfprom_address[1] = 0xFC4B80C8;
#elif defined (CONFIG_ARCH_MSM8226)
	qfprom_address[0] = 0xFC4B80B8;	qfprom_address[1] = 0xFC4B80C0;
#else
	printk("Unknown chipset.");
	return -EINVAL;
#endif	
	...
	for (i = 0; i < 2; i++){
		ret = qfprom_read_one_row(qfprom_address[i], p_buf);

		if(ret != 0){
			printk("%s read address[%d] fail\n", __func__, i);
			kfree(p_buf);
			return -EINVAL;
		}

		sprintf(buf, "%s:%x:%x", buf, p_buf[0], p_buf[1]); // buf + lsb + msb
	}
    kfree(p_buf);
...
static DEVICE_ATTR(rbvalue, S_IWUSR | S_IRUGO, qfprom_rbvalue_show, NULL);
