...
#if defined(ISIS_L2)
					TOUCH_INFO_MSG("<%d> pos[###,###] w_m[###] w_n[###] w_o[###] p[%3d]\n",
							id,ts->ts_data.curr_data[id].pressure);
#else
...
#if defined(ISIS_L2)
			TOUCH_INFO_MSG("%d finger changed : <### -> ###> x[###] y[###] z[%3d]\n",
						total_num,ts->ts_data.curr_data[id].pressure);
#else
...
#if defined(ISIS_L2)
				TOUCH_INFO_MSG("Cur. button -code: ### state: %d, Prev. button -code: ### state: %d\n",
...
#if defined(CONFIG_MACH_MSM8926_F70N_KR)
static ssize_t mfts_enable_show(struct lge_touch_data *ts, char *buf)
{
	int len = 0;
	len = sprintf(buf, "%d\n", ts->mfts_enable);
	return len;
...
#endif
#if defined(CONFIG_MACH_MSM8926_F70N_KR)
static ssize_t mfts_enable_store(struct lge_touch_data *ts, const char *buf, size_t count)
{
	int value = 0;
	sscanf(buf, "%d", &value);
	TOUCH_INFO_MSG("%s = %d \n", __func__, value);
